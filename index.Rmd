...
yr_fem_religion <- lm(F_MEM ~ SECLRSM + YRORGN + SEAT_PMT + DEM + I_QUOTA + LEFTIST + DECSN + MEMCIVIL + MEMSECPR + CIVILCON + F_NOMCPL + F_NOMPMT + F_LEAD + PLUR + WINGS + DEMOCRAC, data = women_religion)%>%
broom::tidy()%>%
  select(term, est=estimate, se=std.error, pval=p.value)
